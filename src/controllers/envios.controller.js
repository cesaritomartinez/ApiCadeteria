...
const { envios, findEnvioById, createEnvio, deleteEnvioById, updateEnvioService } = require("../models/bd");
...
const updateEnvio = (req, res) => {
  ...
  if ([fechaRetiro, horaRetiroAprox, notas, estado].every(v => v === undefined)) {
    res.status(StatusCodes.BAD_REQUEST)
       .json(createError("bad_request",
         "You must send at least one property: fechaRetiro, horaRetiroAprox, notas, or estado."
       ));
    return;
  }
  
  const envioUpdated = updateEnvioService(envioId, fechaRetiro, horaRetiroAprox, notas, estado);

  if (!envioUpdated) {
        res.status(StatusCodes.NOT_FOUND).json(createError("not_found", `Envio with ID ${envioId} not found`))
        return
    }


  if (envioUpdated.error) { // por si la service devuelve errores de validación
    res.status(StatusCodes.BAD_REQUEST)
...
module.exports = { registerEnvio, getAllEnvios, getEnvioById, deleteEnvio, updateEnvio };
